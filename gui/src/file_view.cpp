...
FileView::FileView(std::string path, std::size_t max_print, bool use_wind)
    : path_(std::move(path)) {
    reload(max_print, use_wind);
}

void FileView::reload(std::size_t max_print, bool use_wind) {
    result_ = analyse(path_, max_print, use_wind);
    track_  = 0;
    reset_channel_selection();   // views belong to the analysis that produced them
}

std::string FileView::title() const {
...
float FileView::draw_spectral_controls() {
...
void FileView::draw_channel_picker() {
    const std::vector<channels::Channel>& ch =
        result_.tracks[static_cast<Size>(track_)].channels;
...
void FileView::draw_transform_controls() {
...
void FileView::draw_signals_tab() {
    if (path_.empty()) {
...
void FileView::draw_track_selector() {
